...
def farthestInitialization(kernel, k, constraint):
    """
        ...
        Returns:
            Assignation in k cluster {Array n}
    """
    ...
    number, components = connected_components(positive, directed=False)
    unique, count = np.unique(components, return_counts = True)
    order = np.argsort(count)[::-1]
    components = np.argsort(unique[order])[components] 

    assert number >= k, "Constraint too important for number of cluster"
